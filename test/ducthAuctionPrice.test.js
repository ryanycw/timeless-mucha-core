...
describe("Timeless  Mucha", function() {
    ...
    describe("Mint Dutch Auction Mint", function() {
        it("Mint with wrong amount of ETH", async function() {
            ...
            await network.provider.send("evm_mine");
            price = await genesisPaperToken.getDutchAuctionPrice();
            expect(ethers.utils.formatEther(price)).to.be.equal('0.03');

            await network.provider.send("evm_increaseTime", [120]);
            await network.provider.send("evm_mine");
            price = await genesisPaperToken.getDutchAuctionPrice();
            expect(ethers.utils.formatEther(price)).to.be.equal('0.02');
            ...
            let curTime = await utils.getCurTime();

            await network.provider.send("evm_increaseTime", [1800-(curTime-initTime)]);
            await network.provider.send("evm_mine");
            price = await genesisPaperToken.getDutchAuctionPrice();
            expect(ethers.utils.formatEther(price)).to.be.equal('0.01');
