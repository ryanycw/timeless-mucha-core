...
describe("Timeless  Mucha", function() {
    ...
    describe("Mint Whitelist Mint", function() {
        it("Mint with wrong amount of ETH", async function() {
            ...
            await expectRevert(
                genesisPaperToken.connect(addr1).mintWhitelistGenesisPapers(4, maxQuant, signature, {value: ethers.utils.parseEther('0.05')}),
                "InvalidAmountETH()"
            );

            await expectRevert(
                genesisPaperToken.connect(addr1).mintWhitelistGenesisPapers(4, maxQuant, signature, {value: ethers.utils.parseEther('0.03')}),
                "InvalidAmountETH()"
            );
